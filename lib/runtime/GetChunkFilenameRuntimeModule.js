	...
	generate() {
		...
		if (includeEntries) {
			for (const c of compilation.chunkGraph.getChunkEntryDependentChunksIterable(
				...
				addStaticUrl(c, getFilenameForChunk(c) || filename);
			}
		}
		for (const c of chunk.getAllAsyncChunks()) {
			const chunkFilename = getFilenameForChunk(c);
			if (chunkFilename && chunkFilename !== filename) {
				addStaticUrl(c, chunkFilename);
			}
		}
		...
		const staticUrlsAsMap = staticUrls.size >= 16;
		// if is shorter for 1 - 15 cases
		// it minimizes to `x===1?"...":x===2?"...":"..."`
		const staticUrlsAsIfCascade = !staticUrlsAsMap && staticUrls.size > 0;
